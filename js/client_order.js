...
{
	var toDate = $("#clientOrderToDate").val(); 
	var fromDate = $("#clientOrderFromDate").val();
	...
	jQuery("#client02").jqGrid('navGrid','#pclient02',
	...
	jQuery("#client02Change").change(function()
	{
		var vl = $(this).val();
		if(vl) 
		{
			if(vl == "clear") {
				jQuery("#client02").jqGrid('groupingRemove',true);
			} else {
				jQuery("#client02").jqGrid('groupingGroupBy',vl);
			}
		}
	});
	$("#clientOrderSearchButton").click(function()
	{
		var toDate = $("#clientOrderToDate").val();
		var fromDate = $("#clientOrderFromDate").val();
		$("#client02").jqGrid('setGridParam',{url:"include/orderController.php?ref=orderDetails&toDate="+toDate+"&fromDate="+fromDate,page:1}).trigger("reloadGrid");
	});
});